...
# video = cv2.VideoCapture('result_test_video.mp4')
...
# 20220415_이성현_np객체의 슬라이싱이 안되는 것 같아서 리스트로 변경해서 처리
layers_names_output = \
    [layers_names_all[i - 1] for i in network.getUnconnectedOutLayers().tolist()]
...
while True:
    ...
    if writer is None:
        ...
        writer = cv2.VideoWriter('result-yolo-video.mp4', fourcc, 30,
